...
fn failure(input: &str) -> nom::error::Error<&str> {
...
impl SimpleParser {
    ...
    pub fn parse(input: &str) -> IResult<&str, Vec<Simple>> {
        map(
            many0(alt((
...
impl FullParser {
            ...
            map_res(
                map(
                    verify(
                        ...
                        |contents: &Vec<&str>| contents.len() > 1 || !contents[0].is_empty(),
                    ),
    ...
    fn parse_search(input: &str, last_char: Option<char>) -> IResult<&str, Search> {
        fn button(input: &str) -> IResult<&str, &str> {
            recognize(alt((
                delimited(
                    char('['),
    ...
    fn parse_code_block(input: &str, last_char: Option<char>) -> IResult<&str, CodeBlock> {
        const MARK: &str = "```";
        delimited(
            alt((value((), line_ending), line_begin(last_char))),
            delimited(
                ...
                map(
                    separated_pair(
                        opt(verify(not_line_ending, |s: &str| !s.is_empty())),
                        line_ending,
                        recognize(many1(preceded(
    ...
    fn parse_math_block(input: &str, last_char: Option<char>) -> IResult<&str, MathBlock> {
        const OPEN: &str = r"\[";
        const CLOSE: &str = r"\]";
        delimited(
            alt((value((), line_ending), line_begin(last_char))),
    ...
    fn parse_unicode_emoji(input: &str) -> IResult<&str, UnicodeEmoji> {
    ...
    fn parse_emoji_code(input: &str) -> IResult<&str, EmojiCode> {
        fn side(input: &str) -> IResult<&str, ()> {
    ...
    fn parse_inline_code(input: &str) -> IResult<&str, InlineCode> {
        delimited(
            char('`'),
            map(
                recognize(many1(preceded(
    ...
    fn parse_math_inline(input: &str) -> IResult<&str, MathInline> {
        const OPEN: &str = r"\(";
        const CLOSE: &str = r"\)";
        delimited(
            tag(OPEN),
    ...
    fn parse_mention<'a>(
        ...
        fn valid(input: &str) -> IResult<&str, Mention> {
        ...
        fn possibly_invalid(input: &str) -> IResult<&str, Text> {
            map(
                recognize(tuple((
                    char('@'),
                    take_till1(|c: char| !c.is_ascii_alphanumeric() && !"_-".contains(c)),
    ...
    fn parse_hashtag<'a>(
        ...
        fn inner_item(input: &str, depth: u32, nest_limit: u32) -> IResult<&str, ()> {
            fn nest(input: &str, depth: u32, nest_limit: u32) -> IResult<&str, ()> {
                let depth = depth + 1;
                if depth < nest_limit {
    ...
    fn parse_url<'a>(&self, input: &'a str) -> IResult<&'a str, Url> {
        fn find_url_str(input: &str) -> IResult<&str, &str> {
        ...
        fn url_alt(input: &str) -> IResult<&str, Url> {
            delimited(
                char('<'),
                map(
                    recognize(tuple((
    ...
    fn parse_plain(input: &str) -> IResult<&str, Plain> {
    ...
    fn parse_text(input: &str) -> IResult<&str, Text> {
